        ...
        try:
            with get_db_connection() as conn:
                ...
                cursor.execute(
                    ...
                    INSERT OR REPLACE INTO system_settings (key, value, description, updated_at)
                    VALUES
                        ('payment_method_cash_label', ?, 'Подпись способа оплаты cash', CURRENT_TIMESTAMP),
                        ('payment_method_transfer_label', ?, 'Подпись способа оплаты transfer', CURRENT_TIMESTAMP),
                        ('payment_method_custom_methods', ?, 'Дополнительные способы оплаты (JSON)', CURRENT_TIMESTAMP)
                    """,
                    (cash_label, transfer_label, custom_methods_json),
                )
                cursor.execute(
                    "INSERT OR REPLACE INTO system_settings (key, value, description, updated_at) VALUES ('payment_method_card_label', '', 'Удалён', CURRENT_TIMESTAMP)"
                )
                conn.commit()
        ...
        try:
            with get_db_connection() as conn:
                ...
                cursor.execute('''
                    INSERT OR REPLACE INTO system_settings (key, value, description, updated_at)
                    VALUES 
                        ('vat_enabled', ?, 'Учитывать НДС в расчете зарплаты (1 = да, 0 = нет)', CURRENT_TIMESTAMP),
                        ('vat_rate', ?, 'Ставка НДС в процентах (по умолчанию 0%)', CURRENT_TIMESTAMP)
                ''', (str(int(vat_enabled)), str(vat_rate)))
