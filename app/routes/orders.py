...
@bp.route('/api/datatables/orders', methods=['GET', 'POST'])
@login_required
def api_datatables_orders():
    """Server-side DataTables источник данных для /all_orders?view=registry.

    Поддержка POST: длинные параметры колонок не укладываются в лимит строки запроса gunicorn (4094).
    Фильтры страницы (?status=…) остаются в query string; тело POST — параметры DataTables.
    """
    # GET (query) + POST (form): объединяем, чтобы читать и draw/columns из тела, и фильтры из URL.
    rv = request.values
    draw = int(rv.get('draw', 1))
    start = int(rv.get('start', 0))
    length = int(rv.get('length', 25))
    if length <= 0:
        length = 25
    if length > 200:
        length = 200
    ...
    date_to = rv.get('date_to') or None
    ...
    search_value = (rv.get('search[value]', '') or '').strip()
    ...
    sort_by = 'updated_at'
    sort_order = 'DESC'
    order_col = rv.get('order[0][column]')
    order_dir = (rv.get('order[0][dir]') or 'desc').strip().lower()
    if order_col is not None and order_dir in ('asc', 'desc'):
        try:
            col_index = int(order_col)
            sort_order = 'ASC' if order_dir == 'asc' else 'DESC'
            col_data_key = rv.get(f'columns[{col_index}][data]', '') or ''
    ...
    for o in result['items']:
        ...
        is_pinned = bool(o.get('is_pinned'))
        pin_title = 'Открепить заявку' if is_pinned else 'Закрепить заявку'
        pin_class = 'is-pinned' if is_pinned else ''
        pin_icon = 'fas fa-thumbtack'
        pin_btn = (
            f'<button type="button" class="btn btn-link btn-sm order-pin-btn {pin_class}" '
            f'data-order-db-id="{oid}" data-pinned="{1 if is_pinned else 0}" '
            f'title="{pin_title}" aria-label="{pin_title}">'
            f'<i class="{pin_icon}"></i>'
            '</button>'
        )
        col_id = (
            '<div class="d-flex align-items-start justify-content-between gap-2">'
            f'<div><div class="fw-bold">#{oid}</div>'
...
@login_required
@permission_required('view_orders')
def pin_order_api(order_id):
    """Закрепить заявку в списке (глобально для всех пользователей)."""
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            ...
            if not cursor.fetchone():
                return jsonify({'success': False, 'error': 'Заявка не найдена'}), 404
            # Глобальный режим: на одну заявку храним только один закреп.
            cursor.execute(
                """
                DELETE FROM order_pins
                WHERE order_id = ?
                """,
                (order_id,),
            )
            cursor.execute(
                """
                INSERT INTO order_pins (order_id, user_id)
                VALUES (?, ?)
                """,
                (order_id, current_user.id),
            )
            conn.commit()
        return jsonify({'success': True, 'order_id': order_id, 'pinned': True})
    except Exception as e:
        logger.error("Ошибка закрепления заявки %s: %s", order_id, e, exc_info=True)
        return jsonify({'success': False, 'error': 'Не удалось закрепить заявку'}), 500


@bp.route('/api/order/<int:order_id>/pin', methods=['DELETE'])
@login_required
@permission_required('view_orders')
def unpin_order_api(order_id):
    """Открепить заявку в списке (глобально для всех пользователей)."""
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
            ...
            conn.commit()
        return jsonify({'success': True, 'order_id': order_id, 'pinned': False})
    except Exception as e:
