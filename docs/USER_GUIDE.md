...
3. **Внешний вид и комплектация** — одно поле с тегами (в квитанции тоже одна строка).
4. При необходимости укажите **предварительную стоимость** (оценка для клиента, печатается в квитанции; в кассу не уходит) и **предоплату** / способ оплаты.
5. Сохраните.
...
1. Клиент получает логин (свой телефон) и временный пароль — письмом или от сотрудника (§ 17.4).
...
**Письмом «Заказ принят»** — если у клиента заполнен email. При сохранении новой заявки письмо уходит в фоне и содержит ссылку на портал, логин (телефон) и временный пароль.
...
| Тег ссылки в шаблоне | `##PORTAL_URL##` в Настройки → Шаблоны писем → «Заказ принят» |

Если в своём шаблоне письма тега `##PORTAL_URL##` нет, ссылка не появится — добавьте строку вида `Адрес входа: <a href="##PORTAL_URL##">##PORTAL_URL##</a>` рядом с логином и паролем.

**Вручную** — если email не указан. В карточке клиента нажмите «Редактировать» и в блоке «Пароль для портала» либо посмотрите выданный пароль кнопкой «Показать пароль» (работает, пока клиент его не сменил), либо задайте новый. Объясните клиенту, что это отдельный вход от CRM сотрудников.
