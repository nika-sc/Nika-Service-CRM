...
| Логин (email) | Полный email ящика | `you@bk.ru` |
| Пароль | **Пароль приложения**, не обычный пароль сайта | (из настроек ящика) |
| От кого | `Имя <тот же email, что логин>` или пусто | `Nika CRM <you@bk.ru>` |
| Таймаут | Секунды ожидания SMTP | `3`–`10` |
...
**Как исправить:** в «От кого» укажите тот же email, что в логине (можно с отображаемым именем), или очистите поле и сохраните. В актуальной версии CRM демо-отправитель при отправке игнорируется и подставляется логин; всё равно лучше поправить поле в настройках.
...
**Письма не уходят, в логе `not local sender` / `noreply@example.com`.** В **Настройки → Почта** поле **«От кого»** всё ещё с демо-адресом. Укажите тот же email, что в логине (например `Nika CRM <you@bk.ru>`), или очистите поле. См. § 13.5.
