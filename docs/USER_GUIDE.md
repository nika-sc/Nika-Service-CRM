...
- В диагностике — готовые шаблоны текста по типу и модели. § 5.4.2.
- Блог: [безопасность и скорость](/blog/security-catalog-invoices).

### Что нового в 3.10 (2026-08-20)

- **Шаблоны диагностики** — готовый текст по типу, марке и модели; в окне заявки список подставляет или дописывает текст. § 5.4.2, Настройки → Данные об устройстве.
...
В актуальном bootstrap уже отмечены миграции PostgreSQL **001–022** (включая staff chat, pins, demo visitors, **счета B2B**, диагностику и её историю, связи модели с типом/маркой, шаблоны диагностики). После импорта при старте приложения обычно не остаётся неприменённых SQL-миграций из комплекта дампа. Если в репозитории появились более новые файлы в `postgres_versions/`, они применятся при следующем запуске (`run_migrations.py` / Docker entrypoint / `linux_upgrade.sh`).
...
Офлайн-установщик для Windows 10/11 x64 (встроенные Python 3.12, PostgreSQL 18, демо-база), версия **1.0.6**, дата сборки **2026-08-09**:

1. Скачайте [NikaCRM-Offline-Setup-1.0.6-x64.exe](https://github.com/nika-sc/Nika-Service-CRM/releases/download/windows-setup-1.0.6/NikaCRM-Offline-Setup-1.0.6-x64.exe)  
   или зеркало на [service.nika-crm.ru/downloads/…](https://service.nika-crm.ru/downloads/NikaCRM-Offline-Setup-1.0.6-x64.exe).
...
**Что нового в 1.0.6:** после сохранения SMTP перезапустите службу ярлыком **«Nika CRM — Перезапуск службы»**; плейсхолдеры «От кого» вида `Название вашей компании <ваш@email.ru>`. См. § 13.5 и блог [windows-setup-1-0-6](/blog/windows-setup-1-0-6).
...
1. Текст результата (видит клиент). Можно выбрать **шаблон** сверху окна: пустое поле заполнится текстом шаблона, уже написанное — дополнится с новой строки. Фото не обязательны.
2. При необходимости приложите файлы: JPEG, PNG или PDF, до 5 МБ каждый, не больше 10 на заявку.
3. Нажмите **«Сохранить текст»**.

Шаблоны заводятся в **Настройки → Данные об устройстве → Шаблоны диагностики** (право `manage_settings`): название, текст, при желании тип / марка / модель (пусто = для любых). В заявке сначала предлагаются шаблоны этой модели, затем марки и типа.
...
| Данные об устройстве | Справочники типов/брендов/моделей и **шаблоны диагностики**. Смотреть в заявке может любой сотрудник; **добавлять и менять** — право `manage_settings` |
...
| **3.11** | **2026-08-22** | Безопасность и быстрее разделы; шаблоны диагностики; блог 36 |
