...
- Блог: [безопасность и скорость](/blog/security-catalog-invoices), [письма в заявке](/blog/order-customer-emails).
...
В актуальном bootstrap уже отмечены миграции PostgreSQL **001–024** (включая staff chat, pins, demo visitors, **счета B2B**, диагностику и её историю, связи модели с типом/маркой, шаблоны диагностики, журнал писем клиенту). После импорта при старте приложения обычно не остаётся неприменённых SQL-миграций из комплекта дампа. Если в репозитории появились более новые файлы в `postgres_versions/`, они применятся при следующем запуске (`run_migrations.py` / Docker entrypoint / `linux_upgrade.sh`).
