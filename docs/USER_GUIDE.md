...
В актуальном bootstrap уже отмечены миграции PostgreSQL **001–018** (включая staff chat, pins, demo visitors, **счета B2B**, диагностику и её историю). После импорта при старте приложения обычно не остаётся неприменённых SQL-миграций из комплекта дампа. Если в репозитории появились более новые файлы в `postgres_versions/`, они применятся при следующем запуске (`run_migrations.py` / Docker entrypoint / `linux_upgrade.sh`).
...
1. Текст результата (видит клиент). Фото не обязательны.
...
| **Мои устройства** | `/portal/devices` | Техника и список заявок по каждому устройству (та же карточка с диагностикой) |
...
7. Файлы диагностики и вложения комментариев видит только тот, кому заявка доступна. В кабинете клиент видит только свои заявки и свои фото.
...
**Клиент не видит диагностику в кабинете.** Текст ещё не сохраняли, или клиент смотрит чужую заявку (такого быть не должно). Проверьте карточку заявки в CRM.
