...
10. [Зарплата](#10-зарплата)
11. [Отчёты](#11-отчёты)
12. [Настройки](#12-настройки)
13. [Уведомления CRM](#13-уведомления-crm)
14. [Чат сотрудников](#14-чат-сотрудников)
15. [Глобальный поиск](#15-глобальный-поиск)
16. [Клиентский портал](#16-клиентский-портал)
17. [Мобильный доступ и PWA](#17-мобильный-доступ-и-pwa)
18. [Права доступа и безопасность](#18-права-доступа-и-безопасность)
19. [Советы и FAQ](#19-советы-и-faq)
...
В актуальном bootstrap уже отмечены миграции PostgreSQL **001–010** (включая staff chat и закрепление заявок). После импорта при старте приложения обычно не остаётся неприменённых SQL-миграций из комплекта дампа. Если в репозитории появились более новые файлы в `postgres_versions/`, они применятся при следующем запуске.
...
1. **Верхнее меню (navbar)** — разделы: Заявки, Клиенты, Склад, Магазин, Касса, Отчёты, Зарплата, Настройки, Портал; поиск; колокольчик уведомлений.
2. **Левый узкий сайдбар** — быстрые иконки тех же разделов + **Профиль** (выход) + **Чат**.
...
На узком экране верхнее меню сворачивается в кнопку «гамбургер». Левый сайдбар на очень узких экранах может скрываться — пользуйтесь мобильным меню navbar.
...
Откройте **Отчёты** в navbar или левом сайдбаре. Внутри страниц отчётов — боковое меню.
...
## 16. Клиентский портал

Личный кабинет клиента (не путать с входом сотрудника в CRM).

### 16.1. Вход клиента

1. **Портал** в меню или URL `/portal/login`.
2. Логин/пароль, выданные клиенту в карточке клиента CRM.
3. После входа — дашборд `/portal/dashboard`.

### 16.2. Разделы портала
...
### 18.1. Как устроены права

Каждому действию соответствует permission (например, `view_orders`, `manage_warehouse`). Роль задаёт набор прав; admin может уточнять индивидуально.
...
### 18.3. Публичный демо-логин и безопасность сервера
...
| **2.5.2** | **2026-07-27** | История изменений README: блоки май–июль 2026 (SETUP, лендинг, демо-онлайн, сводка дня, hardening) |
