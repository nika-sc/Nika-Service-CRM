...
### 13.2. Что делает сотрудник перед выдачей доступа

1. Откройте **Клиенты** (`/clients`) и выберите нужного клиента.
2. В карточке клиента задайте пароль для портала (или сгенерируйте и покажите его клиенту).
3. Передайте клиенту телефон и временный пароль; при первом входе клиент меняет пароль.
...
![32 Пароль портала задан](assets/walkthrough/32-portal-password-issued.png)
