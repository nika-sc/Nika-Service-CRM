...
- Схема `public`, согласованная с миграциями PostgreSQL **001–023**.
- В **`schema_migrations_pg`** записаны версии **001 … 023** (все файлы из `postgres_versions/`, без пропусков). После импорта `run_migrations.py` не должен показывать pending. Скрипт `database/add_indexes.py` — наследие SQLite, для этой установки **не** нужен.
- В конце дампа — идемпотентные DDL/seed из `011`–`023` (demo visitors + счета B2B + связи каталога/`shop_sale_id` + предварительная стоимость + правка квитанции + диагностика/фото для ЛК + история диагностики + связи модели с типом/маркой + литерал `\\n` в квитанции + шаблоны диагностики + `is_active` как 0/1 + стартовые тексты).
...
4. **Пустая БД без дампа:** достаточно `DATABASE_URL` — при первом запуске применятся все файлы из `app/database/migrations/postgres_versions/` (001…023 и дальше). Дамп быстрее и сразу даёт демо-логины/справочники.
