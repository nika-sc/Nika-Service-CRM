...
- Схема `public`, согласованная с миграциями PostgreSQL **001–016**.
- В **`schema_migrations_pg`** записаны версии **001 … 016**; в конце дампа — идемпотентные DDL/seed из `011`–`016` (demo visitors + счета B2B + связи каталога/`shop_sale_id` + предварительная стоимость + правка квитанции).
...
4. **Пустая БД без дампа:** достаточно `DATABASE_URL` — при первом запуске применятся все файлы из `app/database/migrations/postgres_versions/` (001…016 и дальше). Дамп быстрее и сразу даёт демо-логины/справочники.
