...
- Схема `public`, согласованная с миграциями PostgreSQL **001–020**.
- В **`schema_migrations_pg`** записаны версии **001 … 020**; в конце дампа — идемпотентные DDL/seed из `011`–`020` (demo visitors + счета B2B + связи каталога/`shop_sale_id` + предварительная стоимость + правка квитанции + диагностика/фото для ЛК + история диагностики + связи модели с типом/маркой + литерал `\\n` в квитанции).
...
4. **Пустая БД без дампа:** достаточно `DATABASE_URL` — при первом запуске применятся все файлы из `app/database/migrations/postgres_versions/` (001…020 и дальше). Дамп быстрее и сразу даёт демо-логины/справочники.
