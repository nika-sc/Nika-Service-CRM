...
- Схема `public`, согласованная с миграциями PostgreSQL **001–014**.
- В **`schema_migrations_pg`** записаны версии **001 … 014**; в конце дампа — идемпотентные DDL/seed из `011`–`014` (demo visitors + счета B2B + связи каталога/`shop_sale_id`).
...
4. **Пустая БД без дампа:** достаточно `DATABASE_URL` — при первом запуске применятся все файлы из `app/database/migrations/postgres_versions/` (001…014 и дальше). Дамп быстрее и сразу даёт демо-логины/справочники.
