...
- Схема `public`, согласованная с миграциями PostgreSQL **001–022**.
- В **`schema_migrations_pg`** записаны версии **001 … 022**; в конце дампа — идемпотентные DDL/seed из `011`–`022` (demo visitors + счета B2B + связи каталога/`shop_sale_id` + предварительная стоимость + правка квитанции + диагностика/фото для ЛК + история диагностики + связи модели с типом/маркой + литерал `\\n` в квитанции + шаблоны диагностики + `is_active` шаблонов как 0/1).
...
4. **Пустая БД без дампа:** достаточно `DATABASE_URL` — при первом запуске применятся все файлы из `app/database/migrations/postgres_versions/` (001…022 и дальше). Дамп быстрее и сразу даёт демо-логины/справочники.
