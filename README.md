...
- Склад и закупки; магазин: частые позиции, чек сразу, запись в кассу
...
Система использует миграции для управления схемой БД. Если вы импортировали **`database/bootstrap/nikacrm_public_sanitized.sql`**, то для PostgreSQL записи в **`schema_migrations_pg`** уже соответствуют версиям **001–020** (включая счета B2B, диагностику и связи модели с типом/маркой) — при старте приложения новые SQL-миграции не потребуются, пока в `postgres_versions/` не появится следующая версия.
