...
Система использует миграции для управления схемой БД. Если вы импортировали **`database/bootstrap/nikacrm_public_sanitized.sql`**, то для PostgreSQL записи в **`schema_migrations_pg`** уже соответствуют версиям **001–022** (включая счета B2B, диагностику, связи модели с типом/маркой и шаблоны диагностики) — при старте приложения новые SQL-миграции не потребуются, пока в `postgres_versions/` не появится следующая версия.
