...
### 2026-04 — чат сотрудников и уведомления

**Интерфейс и логика чата (коммиты `46aa5b3`, `12c1acc`, `ce60711`, `82c71d8` и последующие правки в коде)**

- Плавающая панель на всех страницах с `base.html`; realtime через **Socket.IO** (`/staff-chat`), fallback при обрыве канала.
...
- **Курсор прочитанного**: сохранение «дочитано до сообщения N», строки **«Прочитали»**, кнопка **+N** и API читателей; событие **`read_cursor_updated`** для обновления у других без перезагрузки страницы.
- Миграции БД: **058–061** (SQLite) — чат, реакции, курсоры, Web Push; **005–008** (Postgres) — те же сущности + подписки Push.
- **Уведомления 🔔** — системные (Notification API, не Web Push): учёт открытой панели чата и фокуса вкладки; **Shift+клик** — тест и диагностика; звук входящего; смена **заголовка вкладки** при неактивной вкладке.
- **Web Push 📡** — уведомления при **закрытой вкладке** (при настроенных **VAPID** и пакете **`pywebpush`**): service worker `GET /staff-chat-push-sw.js`, таблица подписок (**061** SQLite / **008** Postgres), переменные `STAFF_CHAT_VAPID_*`, скрипты `scripts/generate_staff_chat_vapid_keys.py` и `scripts/ensure_staff_chat_vapid_env.py`.
- **Лимиты** на частоту отправки, длину текста, число и размер файлов.
...
- Добавлено **закрепление заявок (📌)** в `/all_orders` по аналогии с закреплением чатов.
- База данных: новая таблица `order_pins` (SQLite миграция **062**, PostgreSQL миграция **009**).
- Закреплённые заявки отображаются первыми в реестре для всех пользователей; pin/unpin выполняется через API без перезагрузки страницы.
